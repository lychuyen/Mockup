...
![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_LIST.img01.png)
...
flowchart TD
    subgraph SGU["User"]
        ST((" "))
        A1["(1) Đăng nhập"]
        A2["(2) Chọn Data Maintenance =&gt; Chọn tab Quản lý tàu bay"]
    end
    subgraph SGS["Hệ thống"]
        A3["(3) Hệ thống gọi API =&gt; Hiển thị danh sách Aircraft Type List"]
        EN(((" ")))
    end
    ST --> A1
    A1 --> A2
...
![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_LIST.img02.png)
...
| 2 | ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_LIST.img03.png) | Button |  | Click: refresh màn hình => FE call API lấy lại DS tàu bay mới nhất hiện tại để hiển thị trên giao diện người dùng, nếu có bộ lọc thì giữ nguyên điều kiện lọc   * Trường hợp response API trả về thành công và có dữ liệu: hiển thị danh sách tàu bay vào bảng bên dưới * Ngược lại: (API trả về rỗng hoặc lỗi) Hiển thị bảng danh sách với **chân trang = Tất cả danh sách : 0** |
| 3 | ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_LIST.img04.png) | Button |  | * Tên file tải về: TOSS[_Aircraft Type List_ddmmyyhhss](https://docs.google.com/spreadsheets/d/1htVzBzmkNyh0f7PXmugySx0KOOecKaoT/edit?usp=drive_link&ouid=115403346570548127295&rtpof=true&sd=true) * Nội dung file tải về: tải theo cột dữ liệu view từ bảng danh sách tàu bay |
