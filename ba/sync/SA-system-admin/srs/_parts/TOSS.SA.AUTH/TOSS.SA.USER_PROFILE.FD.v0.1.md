...
| **Trigger** | Người dùng click icon *(hình ảnh minh họa — xem file gốc/Google Doc)*ở góc phải màn hình Home |
...
| **STT** | **Bước** | **Mô tả** |
| --- | --- | --- |
|  | Bước 1 | Người dùng click icon *(hình ảnh minh họa — xem file gốc/Google Doc)*trên màn hình Home |
...
| --- | --- | --- | --- | --- |
|  | *(hình ảnh minh họa — xem file gốc/Google Doc)* | Button | btn_exit | * Click button → Đóng pop-up giao diện xem thông tin user đăng nhập (Personal ìnformation) |
|  | *(hình ảnh minh họa — xem file gốc/Google Doc)* | Button | btn_logout | * Click button → Hỗ trợ đăng xuất tài khoản hiện tại ra khỏi web theo chức năng [Sign out](#_heading=h.gn8dm6y6i6a1) (kích hoạt popup xác nhận Sign out) |
|  | *(hình ảnh minh họa — xem file gốc/Google Doc)* | Button | btn_changePassword | * Button chỉ hiển thị áp dụng cho user là local user * Click button → Hiển thị popup [Đổi mật khẩu](TOSS.SA.CHANGE_PASSWORD.FD.v0.1.md) |
