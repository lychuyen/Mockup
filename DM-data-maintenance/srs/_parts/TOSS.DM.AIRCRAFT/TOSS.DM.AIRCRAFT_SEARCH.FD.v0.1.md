...
![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_SEARCH.img01.png)
...
flowchart TD
    subgraph SGU["User"]
        ...
        A3["(3) Tìm kiếm bất kỳ 1 thông tin trên bộ lọc"]
        D1{"?"}
        A4["(4) Click button Search"]
        A5["(5) Click button Clear Filters"]
    end
    subgraph SGS["Hệ thống"]
        A2["(2) Hệ thống gọi API =&gt; Hiển thị danh sách Aircraft Type List"]
        A6["(6) Hiển thị danh sách tàu bay tìm kiếm"]
        A7["(7) Hệ thống xoá hết toàn bộ thông tin trên bộ lọc"]
        EN(((" ")))
    end
    ST --> A1
    A1 --> A2
...
| 9 | Filter Status | Dropdown (single-select) | filter_status | * Placeholder: Status * Trường để lọc: Tìm kiếm chính xác theo [Status] * Các lựa chọn: Active / Inactive |
| 10 | ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_SEARCH.img03.png) | Button |  | * Click vào ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_SEARCH.img04.png) * Hệ thống thực hiện lọc dữ liệu dựa trên từ khoá * Hệ thống trả về danh sách dữ liệ u phù hợp với từ khoá tìm kiếm |
| 11 | ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_SEARCH.img05.png) | Button |  | * Click vào ![Ảnh minh họa](../_images/TOSS.DM.AIRCRAFT_SEARCH.img06.png) * Hệ thống:   + Xoá nội dung search   + Reset toàn trường lọc đã chọn   + Reset phân trang về trang đầu * Hiển thị lại danh sách ban đầu |
