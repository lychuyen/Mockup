...
> **Phạm vi file:** Feature F01 (nhóm Document) — trích trung thực 100% từ nguồn SRS Flight Load Control do người soạn (CLAUDE.md §0), không suy diễn, không bổ sung. Cờ điểm cần xác nhận: xem CATALOG.md dòng #1. **Đồng bộ lại 2026-07-10 theo Google Doc version 2192:** khối quy tắc đầu bảng "Mô tả chi tiết màn hình" bỏ cụm "và lưu trữ vào Server", bổ sung mới "Quy tắc đặt tên file tài liệu" (cấu trúc tên file .PDF, in hoa).
...
> Chuyển từ ảnh sơ đồ luồng gốc (UML Activity, 3 làn User/TOSS/Hệ thống thứ 3) trong Google Doc nguồn — ảnh gốc lưu tại [`_images/TOSS.FLC.FLIGHT_LIST.sodo-luong.png`](../_images/TOSS.FLC.FLIGHT_LIST.sodo-luong.png).
...
flowchart TD
    ...
    subgraph SG2["TOSS"]
        B2["(2) Gọi API để lấy dữ liệu danh sách chuyến bay và tài liệu"]
        B4["(4) Hiển thị danh sách tất cả danh sách chuyến bay và tài liệu chuyến bay"]
    end
    subgraph SG3["Hệ thống thứ 3"]
        B3["(3) Truy vấn toàn bộ danh sách chuyến bay và tài liệu chuyến bay theo id chuyến bay"]
    end
    End((Kết thúc))
...
**Nguồn trích:** `sec-04-xem-danh-sach-chuyen-bay-va-trang-thai-t.md` (mảnh phân rã h2 từ `VNA.TOSS_SRS_Flight Load Control_v0.1.md`, đã xóa sau khi tách theo Feature — git giữ lịch sử) · CATALOG.md dòng #1. **Đồng bộ lại 2026-07-10 theo Google Doc version 2192** (sửa 2026-07-10T10:57:39Z bởi chuyenly2003; bản phân rã trước theo version 2074).
