...
>
> **⚠ NỘI DUNG MỚI CÓ MỘT PHẦN (giữ nguyên trạng nguồn — CATALOG.md dòng #7, mục 2.2; đã kiểm tra lại 2026-07-10 với Google Doc version 2192: KHÔNG thay đổi so với version 2074, phần thiếu VẪN CHƯA được VNA/VTIT bổ sung):**
...
> Chuyển từ ảnh sơ đồ luồng gốc (UML Activity, 2 làn user/TOSS) trong Google Doc nguồn — ảnh gốc lưu tại [`_images/TOSS.FLC.FUEL_ORDER_DETAIL.sodo-luong.png`](../_images/TOSS.FLC.FUEL_ORDER_DETAIL.sodo-luong.png). **Lưu ý:** bước (1) trong ảnh ghi "chọn phân hệ Flight Load Control" (không nêu rõ chọn tab nào) — khác cả Trigger ("Chọn tab Fuel Order") lẫn Bước 1 của bảng "Mô tả luồng xử lý" bên dưới ("chọn tab Document"); giữ nguyên như ảnh gốc, không tự suy diễn thêm.
...
flowchart TD
    ...
    subgraph SG1["user"]
        ...
        B3["(3) Nhấn vào một bản ghi bất kỳ trên danh sách chuyến bay"]
    end
    subgraph SG2["TOSS"]
        B2["(2) Gọi API để lấy dữ liệu và hiển thị danh sách chuyến bay và thông tin fuel order theo id chuyến bay"]
        B4["(4) Hiển thị view &quot;Chi tiết fuel order chuyến bay&quot;"]
    end
    End((Kết thúc))
...
**Nguồn trích:** `sec-10-xem-details-fuel-chuyen-bay.md` (mảnh phân rã h2 từ `VNA.TOSS_SRS_Flight Load Control_v0.1.md`, đã xóa sau khi tách theo Feature — git giữ lịch sử) · CATALOG.md dòng #7. **Đồng bộ lại 2026-07-10 theo Google Doc version 2192** (sửa 2026-07-10T10:57:39Z bởi chuyenly2003; bản phân rã trước theo version 2074) — nội dung mục này không đổi, phần "Màn hình chức năng"/"Mô tả chi tiết màn hình" vẫn trống.
