...
source_version: "920"
source_modified: "2026-07-10T10:06:29.764Z"
...
> Pull 2026-07-10 (version 920, sửa 2026-07-10T10:06:29.764Z bởi vietanh3796).
...
|  | **Trình soạn thảo văn bản (Text editor):**  *(hình ảnh minh họa — xem file gốc/Google Doc)*   * Cho phép nhập và định dạng văn bản gồm:   + Heading (H1, H2…)   + In đậm (**B**)   + In nghiêng (*I*)   + Gạch bỏ (~~S~~)   + Line Separator (-)   + Text block (“”)   + Chèn liên kết   + Danh sách (có thứ tự và không thứ tự)   + Căn lề (trái, giữa, phải) * **Xử lý:** Hiển thị chính xác định dạng văn bản khi xem trước và sau khi lưu.   + Tự động điều chỉnh kích thước theo nội dung.   + Hỗ trợ hiển thị trên các thiết bị và trình duyệt khác nhau. |
|  | **Tệp đính kèm (Attachments):** Cho phép đính kèm tệp tin vào báo cáo  *(hình ảnh minh họa — xem file gốc/Google Doc)*   * Cách thức upload:   + Kéo thả từ máy tính vào vùng chỉ định   + Nhấn “Add Attachment” để mở hộp thoại chọn tệp từ máy tính. * Tối đa upload cùng 1 lúc: 10 tệp * Kích thước tối đa mỗi tệp: 5MB * Xử lý lỗi:   + Thông báo lỗi nếu định dạng tệp không hợp lệ: *“Unsupported file type. Please upload .xlsx, .docx, .pdf, .jpg, .jpeg, .png, .gif, .mp4* *files.”*   + Thông báo lỗi nếu tệp vượt quá giới hạn kích thước *“The file is too large. Please upload a file smaller than 5MB.”*   + Thông báo lỗi nếu tổng số file vượt quá cho phép: *“Too many files uploaded. Please remove some files before adding more.”*   + Thông báo lỗi nếu số lượng file vượt quá 10 tệp: *“Please upload no more than 10 files at a time.”*   + Thông báo lỗi nếu lỗi xảy ra: *“File upload failed. Please try again later.”* |
|  | **Kịch bản Title của hệ thống:**  ***(hình ảnh minh họa — xem file gốc/Google Doc)*** => **Logo hệ thống** => Click: **Điều hướng trang HOME** của TOSS  *(hình ảnh minh họa — xem file gốc/Google Doc)*  *(hình ảnh minh họa — xem file gốc/Google Doc)*   * Màn Home của hệ thống TOSS bao gồm 4 phân hệ: System Admin (mô tả tại B8)   + System Admin   + Data Maintenance   + Data Source Monitoring   + TOSS (để trống)   Icon System Admin *(hình ảnh minh họa — xem file gốc/Google Doc)*=> **System Admin:**   * + Hiển thị khi người dùng được phân quyền   + Click vào => Hiển thị hệ thống System Admin, đồng thời menu dọc mở ra hiển thị đầy đủ list menu (user có thể thu mở menu), hiển thị màn hình chờ (không focus vào phân hệ nào).   Icon Data Maintenance *(hình ảnh minh họa — xem file gốc/Google Doc)* => **Data Maintenance:**   * + Hiển thị khi người dùng được phân quyền   + Click vào => Hiển thị hệ thống Data Maintenance, đồng thời menu dọc mở ra hiển thị đầy đủ list menu, (user có thể thu mở menu) , default focus phân hệ **đầu tiên trong list menu** khi User truy cập hệ thống Data Maintenance   + Trường hợp User không được phân quyền => ẩn icon Data Maintenance ở màn Home   Icon Data Source Monitoring *(hình ảnh minh họa — xem file gốc/Google Doc)* => **Data Source Monitoring:**   * + Hiển thị khi người dùng được phân quyền   + Click vào => Hiển thị hệ thống Data Source Monitoring, đồng thời menu dọc mở ra hiển thị đầy đủ list menu (user có thể thu mở menu) , default focus phân hệ **đầu tiên trong list menu** khi User truy cập hệ thống Data Source Monitoring   + Trường hợp User không được phân quyền => ẩn icon Data Source Monitoring ở màn Home   Icon TOSS *(hình ảnh minh họa — xem file gốc/Google Doc)* => **TOSS:**   * + Hiển thị khi người dùng được phân quyền   + Click vào => Hiển thị hệ thống TOSS, đồng thời menu dọc mở ra hiển thị đầy đủ list menu (user có thể thu mở menu) , default focus phân hệ **đầu tiên trong list menu** khi User truy cập hệ thống TOSS   + Trường hợp User không được phân quyền => ẩn icon TOSS ở màn Home   *(hình ảnh minh họa — xem file gốc/Google Doc)* => **Chuông thông báo**: Click => Mở màn hình Thông báo hệ thống (Kịch bản màn hình tham chiếu tài liệu **SRS_Notification**)  *(hình ảnh minh họa — xem file gốc/Google Doc)*=> **Thông tin User đăng nhập**   * *(hình ảnh minh họa — xem file gốc/Google Doc)*Avatar User đăng nhập * Di chuột hiển thị tooltip [Xem thông tin cá nhân của người dùng](https://docs.google.com/document/d/1pDG8yL_7-iD9dH030r843FdHB-kTjl9E/edit#heading=h.gn8dm6y6i6a1) |
...
|  | **Quy tắc định dạng số****1 Phạm vi áp dụng** Các quy tắc dưới đây áp dụng cho **toàn bộ trường số trong hệ thống TOSS** — bao gồm cả hiển thị trên giao diện và trong các file xuất (Excel, PDF). **2 Quy tắc định dạng**  | **Quy tắc** | **Giá trị** | | --- | --- | | Dấu phân cách hàng nghìn | Dấu phẩy , | | Dấu phân cách thập phân | Dấu chấm . | | Chuẩn áp dụng | Định dạng số USA |   **Ví dụ minh họa:**   | **Giá trị gốc** | **Giá trị hiển thị** | | --- | --- | | 12000000.68 | 12,000,000.68 | | 73500 | 73,500 | | 97170.5 | 97,170.5 | | 0.5 | 0.5 | | 242 | 242 |  **3 Quy tắc áp dụng chi tiết** **Áp dụng cho:**  · Tất cả trường số nguyên và số thực có thể đạt hoặc vượt giá trị 1000.  · Trường trọng lượng (ví dụ: MTOW tính bằng kg), sức chứa, số lượng, nhiên liệu, và các giá trị tài chính.  · File xuất Excel: áp dụng định dạng ô #,##0.## cho tất cả cột số.  **Không áp dụng cho:**  · Trường mã/ID (ICAO code, IATA code, UUID) — hiển thị nguyên vẹn.  · Tọa độ địa lý (latitude/longitude) — theo quy ước WGS84.  · Trường tỷ lệ phần trăm — định dạng ##.##% (ví dụ: 12.50%).  · Số phiên bản, số điện thoại, và các định danh cấu trúc tương tự.  **Hành vi khi nhập liệu:**  · Hệ thống tự động định dạng giá trị hiển thị khi người dùng rời khỏi ô nhập liệu (out-focus). Giá trị lưu trữ trong cơ sở dữ liệu không bị ảnh hưởng.  · Khi người dùng focus vào ô nhập liệu số, hiển thị giá trị thô (không định dạng) để thuận tiện chỉnh sửa. |

## **Màn lịch sử (Change History) — Quy chuẩn layout**

## **1.2.1 Mục đích**

Màn lịch sử cho phép người dùng xem toàn bộ thay đổi đã xảy ra trên một bản ghi theo thời gian. Mục này định nghĩa **layout giao diện chung** cho tất cả các màn lịch sử trong hệ thống TOSS. Các cột dữ liệu cụ thể hiển thị tại mỗi module phụ thuộc vào trường dữ liệu mà backend của module đó trả về — layout là bắt buộc áp dụng, nhưng tập hợp cột cụ thể linh hoạt theo từng module.

### **1.2.2 Màn hình chức năng**

### ***(hình ảnh minh họa — xem file gốc/Google Doc)***

### **1.2.3 Khu vực bộ lọc (Filter)**

Thanh bộ lọc nằm phía trên bảng dữ liệu, có thể thu gọn/mở rộng thông qua nút “Filter” (kèm icon chevron).

**Trạng thái mặc định:** Mở rộng khi vào màn hình. Tất cả ô lọc để trống đồng nghĩa hiển thị toàn bộ bản ghi.

**Các trường lọc (layout tham chiếu — mỗi module áp dụng trường tương ứng với dữ liệu backend trả về):**

| **Trường** | **Kiểu control** | **Placeholder** | **Ghi chú** |
| --- | --- | --- | --- |
| Khoảng ngày | Date range picker | DD/MM/YYYY → DD/MM/YYYY | Lọc theo thời gian ghi log |
| Người thực hiện | Text input | Search by User name | Tìm kiếm tương đối, không phân biệt hoa thường |
| Loại hành động | Dropdown | Action | Giá trị: CREATE / UPDATE / DELETE |
| Section / Tab | Text input | Search by section | Lọc theo tên section hoặc tab trong module |
| Tên trường | Text input | Search by Field | Lọc theo tên trường dữ liệu bị thay đổi |

Số lượng trường lọc và nhãn tên có thể khác nhau tùy module. Cấu trúc layout (thanh bộ lọc kèm nút Search và Clear Filter, đặt phía trên bảng dữ liệu) nhất quán trên toàn hệ thống.

**Các nút hành động:**

· Nút **Search** — tải lại bảng dữ liệu, quay về trang 1.

· Nút **Clear Filter** — xóa toàn bộ điều kiện lọc, tải lại bảng.

· Nút **Export Excel** — góc trên phải khu vực bộ lọc, xuất kết quả đang lọc ra file Excel (xem quy chuẩn tại Mục III).

### **1.2.4 Bảng dữ liệu**

Bảng hiển thị danh sách log thay đổi. Tập hợp cột tùy thuộc vào từng module và dữ liệu backend trả về. Dưới đây là các cột tham chiếu từ module quản lý tàu bay (module khác có thể có ít cột hơn hoặc tên cột khác):

| **Cột** | **Căn lề** | **Mô tả** |
| --- | --- | --- |
| Ngày/Giờ | Trái | Thời gian ghi log. Định dạng: yyyy-MM-dd • HH:mm. Sắp xếp mới nhất lên trên. |
| Người thực hiện | Trái | Họ tên đầy đủ của tài khoản thực hiện thay đổi. |
| Section | Giữa | Tên section/tab phát sinh thay đổi. Hiển thị dạng **badge** (nền xanh dương nhạt). |
| Hành động | Giữa | Loại hành động: **CREATE** (badge xanh lá) / **UPDATE** (badge cam) / **DELETE** (badge đỏ). |
| Trường dữ liệu | Trái | Tên trường bị thay đổi. Mỗi trường hiển thị trên một dòng riêng với dấu chấm xanh (•) ở đầu. |
| Giá trị cũ | Trái | Giá trị trước khi thay đổi. Hiển thị dạng **gạch ngang** (strikethrough). Nếu không có, hiển thị —. |
| Giá trị mới | Trái | Giá trị sau khi thay đổi. Hiển thị **in đậm**. Nếu không có, hiển thị —. |

**Quy tắc gom nhóm hàng:** Các thay đổi thuộc cùng một lần thao tác (cùng Ngày/Giờ + Người thực hiện + Hành động) được gom thành một nhóm hàng. Các ô Ngày/Giờ, Người thực hiện, Section và Hành động được merge trong nhóm đó. Mỗi trường bị thay đổi hiển thị trên một dòng con riêng trong nhóm.

**Trạng thái rỗng:** Nếu không có bản ghi nào khớp bộ lọc, hiển thị thông báo trong vùng bảng: “No data available.”
...
| --- | --- | --- | --- | --- |
|  | Export Excel | Button | btn_export_excel | * Tên button: Export Excel * Vị trí button: Góc trên phải bảng dữ liệu, trên màn hình danh sách * Chức năng khi click: Gửi yêu cầu export và tải file xlsx về * Trạng thái button: * Enabled nếu có dữ liệu * Disabled nếu không có dữ liệu * Có loading khi đang xử lý * Thông báo lỗi/thành công:   + Thành công, hiển thị thông báo [TB004](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.og0qzr36ep37)   + Timeout/mất kết nối CSDL, hiển thị thông báo [TB005](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.6i14i6qmgfmw) * Tên tải xuống [module_ddMMyyhhmm.xlsx] (Trong đó ddMMyyyyhhmm là ngày giờ tải file) * Tải xuống theo template |
|  | Export pdf | Button | btn_export_pdf | * Tên button hiển thị (label): Export PDF * Vị trí button: Góc trên phải bảng dữ liệu, trên màn hình danh sách * Chức năng khi click: Gửi yêu cầu export và tải file PDF về * Trạng thái button: luôn enabled   + TH k có dữ liệu vẫn cho export => file excel sẽ không có dữ liệu * ~~Enabled nếu có dữ liệu~~ * ~~Disabled nếu không có dữ liệu~~ * Có loading khi đang xử lý * Thông báo lỗi/thành công:   + Thành công, hiển thị thông báo [TB004](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.og0qzr36ep37)   + Timeout/mất kết nối CSDL, hiển thị thông báo [TB005](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.6i14i6qmgfmw) * Tên tải xuống [module_ddMMyyyyhhmm.pdf] (Trong đó ddMMyyyyhhmm là ngày giờ tải file) * Tải xuống theo template |

## **1.4.5 Quy chuẩn file xuất Excel (.xlsx)**

### **1.4.5.1 Quy tắc đặt tên file**

| **Yếu tố** | **Quy tắc** |
| --- | --- |
| Cú pháp | TOSS_Module_Name_DDMMYYYYhhmmss.xlsx |
| Tiền tố | Luôn là TOSS_ |
| Tên module | PascalCase, các từ nối bằng dấu gạch dưới _ |
| Timestamp | Ngày giờ xuất file: DD (ngày 2 chữ số) + MM (tháng 2 chữ số) + YYYY (năm 4 chữ số) + hh (giờ 24h) + mm (phút) + ss (giây) |
| Ví dụ | TOSS_Aircraft_Type_List_10072026143700.xlsx |

### **1.4.5.2 Cấu trúc file**

## Mỗi file xuất gồm một sheet đặt tên theo module (ví dụ: “Aircraft List”).

## **Bố cục theo hàng:**

## **Hàng 1–3** : Vùng dự trữ (đặt logo nếu có) **Hàng 4** : Hàng metadata — thông tin xuất file **Hàng 5** : Hàng tiêu đề báo cáo **Hàng 6** : Hàng tiêu đề cột **Hàng 7+** : Hàng dữ liệu

## **1.4.5.2 File mẫu:** [**TOSS_Aircraft_Type_List_ddmmyyhhss.xlsx**](https://docs.google.com/spreadsheets/d/1htVzBzmkNyh0f7PXmugySx0KOOecKaoT/edit?gid=1072656901#gid=1072656901)
...
| **STT** | **Tên** | **Kiểu dữ liệu**  **[Độ dài dữ liệu]** | **Mapping DB/API** | **Mô tả nghiệp vụ** |
| --- | --- | --- | --- | --- |
|  | Choose file *(hình ảnh minh họa — xem file gốc/Google Doc)* | Button [0;5MB] | btn_import_choose_file | * Tên nút: Choose file * Vị trí: Nằm ở giữa popup * Trạng thái mặc định: Luôn bật (enabled) * Khi nhấn nút: giao diện mở hộp thoại file picker. Chỉ cho phép chọn các file định dạng được hỗ trợ * File tải lên tối đa 5MB * Chỉ được tải lên 1 file trong 1 lần tải, hỗ trợ định dạng .xls, .xlsx * Trường hợp lỗi và thông báo hiển thị:   + Nếu chưa chọn file tải lên, thông báo [TB013](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.vna5y6tbq0zb)   + File lớn hơn giới hạn, hiển thị toast message [TB014](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.6szt7q8qjhsz)   + Nếu file tải lên không đúng định dạng .xlsx, hiển thị toast message [TB015](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.86grxwfg9njj)   + Nếu định dạng đã đúng theo biểu mẫu thì hệ thống sẽ tiếp tục validate toàn bộ dữ liệu trong file import có thỏa mãn các điều kiện sau:     - Không để trống các cột bắt buộc     - Không trùng mã     - Định dạng ngày tháng đúng     - Định dạng email/số điện thoại hợp lệ     - Giá trị trong danh mục hợp lệ   + BE trả về response     - Import thành công toàn bộ: {   "status": "success",  "data.message": "Import dữ liệu thành công.",  "data.imported_count": [số lượng dòng đã import thành công]  }   * + - Import thành công một phần: {   "status": "partial",  "data.message": "Import hoàn tất với một số dòng lỗi.",  "data.imported_count": [số lượng dòng đã import thành công],  "data.error_count": [số lượng dòng bị lỗi],  "data.errors": [  {  "row": "[số dòng lỗi trong file]",  "column": "[tên cột lỗi]",  "message": "[nội dung lỗi]"  }  Import thất bại toàn bộ: {  "status": "fail",  "data.message": "Dữ liệu trong file không hợp lệ.",  "data.imported_count": 0,  "data.errors": [  {  "row": "[số dòng lỗi]",  "column": "[tên cột lỗi]",  "message": "[nội dung lỗi]"  }   * + FE xử lý response     - Import thành công toàn bộ:   Hiển thị toast message [TB016](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.rmd1idswiear) (nội dung lấy từ data.message)  Giao diện import đóng (nếu có popup)  Reload lại danh sách dữ liệu nếu có   * + - Import thành công một phần:   Hiển thị toast message: [TB017](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.qkmegfo2xw2d) (từ data.message)  Giao diện hiển thị:   * + - * Tổng số dòng đã import thành công: lấy từ data.imported_count       * Tổng số dòng bị lỗi: lấy từ data.error_count   Chỉ ghi dữ liệu đã import thành công, không lưu dữ liệu bị lỗi.   * + - Import thất bại toàn bộ:   Hiển thị toast lỗi: [TB018](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.o4xi9sf7c4ep) (từ data.message)  Giao diện hiển thị: danh sách trắng, không có dòng nào được ghi |
|  | Download template file | Button link | btn_import_download_template | * Tên nút: Download template file * Vị trí: Nằm ở khu vực hướng dẫn * Trạng thái mặc định: Luôn bật (enabled) * File tải về có tên: template_import_[module].xlsx * Trường hợp lỗi và thông báo hiển thị: ”   + Mạng lỗi, hiển thị thông báo [TB006](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.ezrmpmpidug)   + Tải file thành công, hiển thị thông báo [TB007](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.cnobkg94hlw5) |
| **Action**   * Cho phép user kéo thả file vào vùng import   *(hình ảnh minh họa — xem file gốc/Google Doc)*   * Sau khi file được chọn tải lên/kéo thả vào vùng import => hệ thống thực hiện tiến trình tải dữ liệu file lên hệ thống,   *(hình ảnh minh họa — xem file gốc/Google Doc)*  đồng thời check valid các trường trong file, nếu:   * + Valid file tải lên thành công: hiển thị màn hình   *(hình ảnh minh họa — xem file gốc/Google Doc)*   * + - Click **Cancel**: Đóng popup và không cần xử lý gì     - Click **Continue**: hệ thống thực hiện tiến trình update thông tin cho danh mục theo thông tin trên file   *(hình ảnh minh họa — xem file gốc/Google Doc)*  Nếu:   * + - * update thành công (xử lý update thông tin các bản ghi và người dùng tương ứng theo KB sửa): hiện toast báo thành công [TB019](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.jsiuauzgpaj1)       * ngược lại: hiện toast lỗi [TB020](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.89c7c2lipqad)   Hoặc [TB021](https://docs.google.com/document/d/14u5WJYigaomf4MupIIDxPeayYOf3_zq-/edit#bookmark=id.97ib6b4szg04)   * + Ngược lại: hiện thống báo lỗi   *(hình ảnh minh họa — xem file gốc/Google Doc)*  Trong đó:   * + - Số bản ghi thành công: = [SL bản ghi thỏa màn valid]/Tổng số bản ghi tải lên     - Số bản ghi thất bại: = [SL bản ghi **không** thỏa màn valid]/Tổng số bản ghi tải lên     - File result: *(hình ảnh minh họa — xem file gốc/Google Doc)* => click vào > thực hiện tải file kết quả về thiết bị theo mẫu template_import_[module].xlsx | | | | |
...
* + Hiển thị: “Tất cả danh sách: X” mà ở đó X là tổng số bản ghi
  ...
  + Mặc định tối đa 10 bản gh i trên một trang
  + Tùy chọn số lượng bản ghi qua dropdownlist: 10, 25, 50, 100.
  + Nút điều hướng:
    - *(hình ảnh minh họa — xem file gốc/Google Doc)*: Di chuyển đến trang đầu
    - *(hình ảnh minh họa — xem file gốc/Google Doc)*: Di chuyển đến trang trước
    - *(hình ảnh minh họa — xem file gốc/Google Doc)*: Hiển thị số trang hiện tại của bảng dữ liệu
    - *(hình ảnh minh họa — xem file gốc/Google Doc)*: Di chuyển trực tiếp đến trang mong muốn
  ...
  + *(hình ảnh minh họa — xem file gốc/Google Doc)*: Di chuyển đến trang sau
  + *(hình ảnh minh họa — xem file gốc/Google Doc)*: Di chuyển đến trang cuối
...
* Cơ chế Thu gọn/Mở rộng bộ lọc (Collapsible Fillter):
  ...
  + **Thao tác Thu / Mở:**
    - * Bấm icon *(hình ảnh minh họa — xem file gốc/Google Doc)* => Thu gọn bộ lọc (chỉ còn thanh Header Bộ lọc tìm kiếm ▼), tự động kéo bảng danh sách tàu bay phía dưới rộng lên trên.
      * Bấm icon *(hình ảnh minh họa — xem file gốc/Google Doc)* => Thả cụm bộ lọc xuống lại bình thường.
