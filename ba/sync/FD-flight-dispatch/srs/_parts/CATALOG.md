...
## 0. Ghi chú nguồn (sec-00 → sec-03 — front matter)

- **sec-00** — Trang bìa + mục lục: mã hiệu dự án `VNA.FIMS`, mã hiệu tài liệu `VNA.FIMS_SRS_Flight Dispatch_v0.1`; Bảng ghi nhận thay đổi tài liệu và Trang ký còn để trống (mẫu biểu Viettel VTIT chưa điền).
- **sec-01** — Mục đích tài liệu SRS: 4 gạch đầu dòng chuẩn (xác định phạm vi hệ thống, làm cơ sở thống nhất VNA–VTIT, định hướng thiết kế/phát triển, cơ sở kiểm thử/nghiệm thu); ghi rõ tài liệu **không** mô tả kiến trúc hệ thống chi tiết, không bao gồm kế hoạch triển khai/vận hành.
- **sec-02** — Phạm vi tài liệu: bảng đối tượng đọc (6 vai trò: BA, Dev, QA/Tester, PM, Đơn vị vận hành, Đại diện VNA) + bảng phạm vi hệ thống Phase 1 (5 phân hệ, đều đánh dấu "✅ Trong phạm vi": HOME — Đăng nhập & Quản lý phiên; Phân hệ Quản lý Điều hành Bay (TOSS); Phân hệ Danh mục dùng chung; Quản trị hệ thống; Báo cáo).
- **sec-03** — Khái niệm/thuật ngữ: bảng 7 thuật ngữ (TT/STT, VNA, FIMS, CFP, OFP, e-CFP/OFP, PIC); dòng **OFP để trống cột Khái niệm** [Cần làm rõ: chưa có định nghĩa].
...
|---|---|---|---|---|---|---|---|---|
| 1 | Danh sách Flight Plan | Danh sách | [TOSS.FD.FLIGHT_PLAN_LIST.FD.v0.1.md](TOSS.FD.FLIGHT_PLAN_LIST.FD.v0.1.md) | Xem danh sách Flight Plan | Truy cập module TOSS → Flight Dispatch → tab Flight Plan | 33 | 3 | Toàn bộ 33 trường không có mapping DB/API; nút Action "View Briefing Sheet" trỏ liên kết nội bộ "Xem chi tiết Flight Plan" nhưng tên section thực tế trong tài liệu là "Xem chi tiết Briefings sheet" [Cần làm rõ: tên liên kết không khớp tiêu đề section]; export "TOSS_FlightPlan_ddmmyyhhmm" trỏ Google Sheets |
| 2 | Xem chi tiết Briefings sheet | Xem | [TOSS.FD.BRIEFING_DETAIL.FD.v0.1.md](TOSS.FD.BRIEFING_DETAIL.FD.v0.1.md) | Xem chi tiết Briefing Sheet của Flight Plan được chọn (suy từ luồng — xem cờ) | Chọn 1 Flight Plan trong danh sách (suy từ Bước 3 luồng nghiệp vụ) | 4 | 4 | Tiêu đề mục "Màn hình chức năng" vẫn ghi **"(chưa chốt)"**; bảng 4 dòng STT (1-4) nhưng toàn bộ cột Tên/Kiểu dữ liệu/Mapping/Mô tả đều **trống** — không thay đổi so với bản trích trước; nguồn không có bảng Mục đích/Trigger/Tiền điều kiện/Hậu điều kiện riêng (khác chuẩn sec-04, sec-07→11) [Cần làm rõ] |
| 3 | Tìm kiếm Flight Plan | Action (tìm kiếm) | [TOSS.FD.FLIGHT_PLAN_SEARCH.FD.v0.1.md](TOSS.FD.FLIGHT_PLAN_SEARCH.FD.v0.1.md) | Tìm kiếm/lọc danh sách Flight Plan theo nhiều tiêu chí (suy từ luồng — xem cờ) | Nhập điều kiện lọc và nhấn Search (suy từ Bước 3-4 luồng nghiệp vụ) | 16 | 7 | Nguồn không có bảng Mục đích/Trigger riêng (khác chuẩn sec-04, sec-07→11) [Cần làm rõ]; trường ARR (dropdown) mô tả nhầm "hiển thị danh sách các mã sân bay khởi hành (ARR)" — nghi sao chép từ mô tả DEP (ARR phải là sân bay đến); toàn bộ không có mapping DB/API |

**UPLOAD DOCUMENT (sec-07 → sec-11)**

| # | Chức năng | Loại | Section | Mục đích (rút gọn) | Trigger (rút gọn) | Số trường | Số bước | Cờ |
|---|---|---|---|---|---|---|---|---|
| 4 | Danh sách tài liệu chung chuyến bay | Danh sách | [TOSS.FD.COMMON_DOCS_LIST.FD.v0.1.md](TOSS.FD.COMMON_DOCS_LIST.FD.v0.1.md) | Xem/tìm kiếm danh sách tài liệu dùng chung chuyến bay theo khoảng hiệu lực | Click tab "Tài liệu chung" trong màn hình Upload Document | 12 | 5 | Bảng danh sách tài liệu nhảy STT từ 1 sang 3 (**thiếu STT 2**); toàn bộ không có mapping DB/API |
| 5 | Danh sách tài liệu theo từng chuyến | Danh sách | [TOSS.FD.FLIGHT_DOCS_LIST.FD.v0.1.md](TOSS.FD.FLIGHT_DOCS_LIST.FD.v0.1.md) | Xem/tìm kiếm danh sách tài liệu theo từng chuyến bay | Truy cập Flight Dispatch → Upload Document → tab Tài liệu chuyến bay | 19 | 5 | Bảng danh sách nhảy STT từ 9 sang 11 (**thiếu STT 10**); cột "Upload date" (STT 6) mô tả nội dung lại là "Operating Date... định dạng dd/MM" (nghi sao chép từ cột DATE ở sec-04, nhãn cột và mô tả không khớp) [Cần làm rõ]; toàn bộ không có mapping DB/API |
| 6 | Upload tài liệu theo từng chuyến bay | Action (upload) | [TOSS.FD.UPLOAD_FLIGHT_DOC.FD.v0.1.md](TOSS.FD.UPLOAD_FLIGHT_DOC.FD.v0.1.md) | Upload tài liệu cho 1 chuyến bay cụ thể | Chọn chuyến bay → nhấn "Choose file" tại bản ghi tương ứng | 7 | 9 | Mục đích ghi lặp từ "Upload tài liệu theo từng chuyến bay **chuyến bay**" (giữ nguyên văn nguồn); cột Bước ghi số thuần (1-9) không có tiền tố "Bước" (khác định dạng sec-04/06/07/08) [ghi chú định dạng]; toàn bộ không có mapping DB/API |
| 7 | Upload tài liệu chung chuyến bay | Action (upload) | [TOSS.FD.UPLOAD_COMMON_DOC.FD.v0.1.md](TOSS.FD.UPLOAD_COMMON_DOC.FD.v0.1.md) | Upload tài liệu dùng chung, áp dụng cho nhiều chuyến bay có ETD trong khoảng hiệu lực khai báo | Nhấn nút "Upload tài liệu chung" | 7 | 9 | Cột Bước ghi số thuần (1-9) không có tiền tố "Bước" (khác định dạng sec-04/06/07/08) [ghi chú định dạng]; toàn bộ không có mapping DB/API |
| 8 | Xoá tài liệu chung chuyến bay | Xóa | [TOSS.FD.DELETE_COMMON_DOC.FD.v0.1.md](TOSS.FD.DELETE_COMMON_DOC.FD.v0.1.md) | Xóa 1 hoặc nhiều tài liệu chung chuyến bay (kèm popup xác nhận) | Tích chọn tài liệu cần xóa hoặc nhấn icon Xóa tại dòng tương ứng (theo Bước 1 luồng thực tế) | 6 | 4 | Trigger ghi trong bảng đầu mục là "mở Flight Dispatch → Upload Document" (giống hệt các chức năng upload khác) nhưng **không khớp** Bước 1 thực tế của luồng nghiệp vụ (tích chọn tài liệu/nhấn icon Xóa) [Cần làm rõ]; toàn bộ không có mapping DB/API |
...
- **sec-07** (bảng danh sách tài liệu chung): STT nhảy từ 1 sang 3, thiếu dòng số 2.
- **sec-08** (bảng danh sách tài liệu theo chuyến): STT nhảy từ 9 sang 11, thiếu dòng số 10.
...
- **sec-08**, cột "Upload date" (STT 6 trong bảng danh sách): nội dung mô tả nghiệp vụ lại là "Hiển thị ngày khai thác (Operating Date) của chuyến bay theo định dạng dd/MM (ví dụ: 24 Jun)" — không khớp với tên cột "Upload date"; nghi sao chép từ cột DATE ở sec-04.
...
sec-04, trường "Action" (STT 31): mô tả nút "View Briefing Sheet" trỏ liên kết nội bộ `[Xem chi tiết Flight Plan](#_30y9z1lnluwm)`, nhưng tên section đích thực tế trong tài liệu là "**Xem chi tiết Briefings sheet**" (sec-05) — tên liên kết và tên section không khớp nhau.
