...
1. **Nguồn khác 14 danh mục còn lại — đã tra ngược, kết quả hỗn hợp (2026-07-15):** mọi file dẫn nguồn `BR-420` (Business Requirement), không phải `sec-NN` của `VNA.TOSS_SRS_Data Maintenance_v0.1.docx` như 14 danh mục khác.
   - **BR-420 ở mức yêu cầu là chính thức**, không phải nội dung tự bịa: đăng ký trong [TOSS.DM.ALL.BRD.v0.6.md](../brd/TOSS.DM.ALL.BRD.v0.6.md) dòng #52 (v0.6, 2026-06-23 — có trước đợt làm việc APU 2026-07-15), nguồn ghi "YCKT TOSS-127, AC APU INOP" — cả 2 đều là dòng thật trong sheet YCKT gốc của khách hàng (`ba/workspace/drafts/phan-tich/03-yckt/sheet-04-yckt-v3.md` dòng #167 "AC APU INOP" + #168 "TOSS-127"); cũng xuất hiện trong Function List chính thức (`VNA-TOSS-Function-list-v1.0.extracted.md`, mục "APU INOP" — kế hoạch Tháng 9) và sheet tiến độ sản xuất Sprint 2 (`ba/sync/quan-ly-yeu-cau/SPRINT2-full-list.md`, rows 58–62, đã liệt kê sẵn đúng 5 chức năng, cột SRS = ❌ trước khi nhóm này được soạn).
   - **Phần còn "nháp":** không phải BR-420, mà là **đặc tả SRS chi tiết từng màn hình** (field/luồng/validate của LIST/CREATE/EDIT/DELETE/EXPORT) — khác 14 danh mục kia (trích trực tiếp từ SRS Word do VNA/VTIT soạn), 5 file này viết trực tiếp từ nội dung BR-420 (chỉ 2 câu) + chỉ đạo nghiệp vụ trực tiếp của BA Lead (state machine, mã định danh) — **chưa có bằng chứng đã qua vòng soát xét chính thức của VNA/VTIT ở mức chi tiết màn hình này.** [Cần làm rõ: cần gửi VNA/VTIT soát xét trước khi coi là hoàn chỉnh ngang hàng 14 danh mục khác?]
