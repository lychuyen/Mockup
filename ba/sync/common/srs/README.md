...
# Ghi chú dùng chung cho SRS mọi module (`common/srs/`)

`common/srs/` **không phải nội dung "không thuộc module nào"** — đây là nơi **mô tả MỘT LẦN** những nội dung mà nhiều module SRS cùng cần tham chiếu (thiết kế UI/UX dùng chung, quy tắc trình bày chung...), để mỗi module không phải lặp lại cùng một mô tả. Ghi chú dưới đây áp dụng cho SRS ở **mọi module** dưới `ba/sync/<module>/srs/`.
...
| `FD-flight-dispatch/srs/` | `VNA.TOSS_SRS_Flight Dispatch_v0.1.md` | Google Doc live (ttphuong060403) | Live — 8 chức năng |
| `FLC-flight-load-control/srs/` | `VNA.TOSS_SRS_Flight Load Control_v0.1.md` | Google Doc live (lyphat676) | Live — 10 chức năng |
| `common/srs/` (đây) | `TOSS.HOME.ALL.FD.v0.1.md` | Google Doc live "Thiết kế dùng chung" (vietanh3796) | Live, mô tả 1 lần dùng chung cho nhiều module Function-list |
| `LO-live-operations/srs/` | `VNA.TOSS_SRS_Live Operations_v0.1.md` | Google Doc live (hiepdv0695) | Live, **hiện trống** — chờ VNA/VTIT soạn |
...
**6 module còn lại của Function-list chưa có SRS:** Station Manager, Report, Data Source Monitoring, Mail cảnh báo, Job đồng bộ, Authentication *(Home = nội dung đã nằm trong `common/srs/`, không tính riêng)*. Xem đủ 12 module tại [`../../README.md`](../../README.md) (root).
...
⚠️ **Phát hiện cần BA Lead xác nhận (2026-07-10):** SRS Flight Load Control (bảng thuật ngữ, mục Khái niệm/thuật ngữ) định nghĩa **FIMS = "OPERATION DATA LAKE/PLATFORM"** — khác cách hiểu "FIMS = TOSS" đã chốt. Giữ nguyên văn nguồn, chưa tự diễn giải quan hệ giữa 2 cách dùng — xem `../../FLC-flight-load-control/srs/_parts/TOSS.FLC.THONG_TIN_CHUNG.FD.v0.1.md`.
...
*v0.1 — 2026-07-10: dựng lại tại `common/srs/` sau khi tái cấu trúc `ba/sync/` theo Option C (1 thư mục/module kỹ thuật). Nội dung kế thừa từ `sync/requirements/srs/README.md` (Option B, xóa).*
